...
指数计算使用加权压力值，较早下课的人群按 0.5 计入，避免早一波学生已分流后仍被等权高估：

```
午饭压力值 = node_flow[4] * 0.5 + node_flow[5]
晚饭压力值 = node_flow[8] * 0.5 + node_flow[9]
```

### 5.4 归一化（0-100 分）

采用**分组自适应归一化**：优先使用过去 30 天内同校区、同餐次、同星期的压力值 P10/P90 作为动态阈值；同星期样本不足时，回退到同日类型（工作日 / 周末）；仍不足时再回退到全量历史。P10/P90 至少保留 600 的压力值跨度，防止低课日几十人的小波动被放大成拥挤。
...
ADAPTIVE_MIN_SPAN = 600

adaptive_score = min(100, max(0, (pressure - P10) / (P90 - P10) * 100))
```

同时保留一个绝对压力下限，避免实际人流不小时仅因低于历史 P10 被硬压成 0 分：

```python
absolute_floor = (pressure - MIN_CROWD) / (MAX_CROWD - MIN_CROWD) * 100 * 0.8
score = max(adaptive_score, absolute_floor)
```
